...
unsigned long long int llist_double_size(llist_double *llist) {
  return llist->len;
}
...
int llist_ul_push_back(llist_ul *llist, unsigned long long data) {
...
unsigned long long int llist_ul_size(llist_ul *llist) {
  return llist->len;
}

unsigned long long llist_ul_back(llist_ul *llist) {
  return llist->last->data;
}
...
void llist_ptr_print(llist_ptr *llist) {
  ...
  cur = llist->first;
  while (cur != NULL) {
    printf("%llx ", (long long unsigned int) cur->data);
    cur = cur->next;
  }
  printf("\n");
}
...
unsigned long long int llist_ptr_size(llist_ptr *llist) {
  return llist->len;
}
